    ...
    mSelectionModel(nullptr)
{
    ...
    connect(ui->previousButton, &QPushButton::clicked,
        ...
        QModelIndex previousModelIndex = mSelectionModel->model()
                ->index(currentModelIndex.row() - 1, 0);
...
void PictureWidget::loadPicture(const QItemSelection &selected, const QItemSelection &deselected)
{
    Q_UNUSED(deselected)
    ...
    mPixmap = QPixmap(mModel->data(current, Qt::DisplayRole)
                      .toString());
    updatePicturePixmap();
    ui->previousButton->setEnabled(current.row() > 0);
    ui->nextButton->setEnabled(current.row() < (
                                   mModel->rowCount() - 1));
...
void PictureWidget::updatePicturePixmap()
{
    ...
    ui->pictureLabel->setPixmap(mPixmap.scaled(
