...
QVariant ThumbnailProxyModel::data(
        const QModelIndex &index, int role) const
{
    ...
    QString filepath =
            sourceModel()->data(index, PictureModel::Roles::FilePathRole)
            .toString();
        ...
        const QModelIndex startIndex, int count)
{
    ...
    for(int row = startIndex.row(); row < lastIndex; row++){
            QString filepath = model->data(model->index(row, 0), PictureModel::Roles::FilePathRole)
                    .toString();
            if(!filepath.isEmpty()){
